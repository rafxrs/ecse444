...
#define NUM_CUES          30
#define CUE_DELAY_MS      50      // 2 seconds to react (comment is stale)
...
/* USER CODE BEGIN PV */
static GameState g_state = GAME_AWAKE;
static uint16_t  noteBuf[NOTE_MAX_SAMPLES];
static uint32_t  noteLen = 0;
...
static volatile int32_t threshold;
...
/* Read all stored records and rebuild stats struct */
static void Flash_ReadStats(GameStats *s)
{
    GameStats temp;
...
/* Append new score */
static void Flash_WriteStats(GameStats *s)
{
