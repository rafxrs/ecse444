...
/* USER CODE BEGIN PV */
volatile uint8_t sensor_mode = 0; // 0=temp, 1=pressure, 2=mag, 3=accel
...
//  /* Erase one 64 KB block */
//  if (BSP_QSPI_Erase_Block(addr) != QSPI_OK)
//      Error_Handler();
//
//  /* Wait for erase to finish (non-blocking) */
//  while (BSP_QSPI_GetStatus() != QSPI_OK);
//
//  /* Write data */
//  if (BSP_QSPI_Write(tx_buffer, addr, sizeof(tx_buffer)) != QSPI_OK)
//      Error_Handler();
//
...
//
//  /* Read data back */
//  if (BSP_QSPI_Read(rx_buffer, addr, sizeof(rx_buffer)) != QSPI_OK)
//      Error_Handler();
//
//  /* Compare buffers and print result */
//  if (memcmp(tx_buffer, rx_buffer, sizeof(tx_buffer)) == 0)
//  {
//      HAL_UART_Transmit(&huart1, (uint8_t *)"QSPI PASS\r\n", 11, 100);
//      HAL_GPIO_WritePin(LED_OK_GPIO_Port, LED_OK_Pin, GPIO_PIN_SET);     // Green ON
//      HAL_GPIO_WritePin(LED_ERR_GPIO_Port, LED_ERR_Pin, GPIO_PIN_SET);   // Red OFF
//  }
//  else
//  {
//      HAL_UART_Transmit(&huart1, (uint8_t *)"QSPI FAIL\r\n", 11, 100);
